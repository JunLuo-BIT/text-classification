...
tf.flags.DEFINE_string("negative_data_file", "./../../data/rt-polarity/rt-polarity.neg", "Data source for the negative data.")
...
# 遍历每行，得到每行的最大单词个数，不足的用 0 填充，生成单词字典
...
# 10 折交叉验证
dev_sample_index = -1 * int(FLAGS.dev_sample_percentage * float(len(y))) #负数：从后往前取
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        cnn = TextCNN(
            ...
            embedding_size=FLAGS.embedding_dim,           # embedding维度
